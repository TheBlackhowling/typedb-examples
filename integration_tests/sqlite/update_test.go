...
// TestSQLite_Update_PartialUpdate_ZeroValues verifies that partial update correctly writes
// zero values (false, 0, "", 0.0) for value types instead of NULL.
func TestSQLite_Update_PartialUpdate_ZeroValues(t *testing.T) {
	db := setupTestDB(t)
	defer closeDB(t, db)
	defer os.Remove(getTestDSN())
	...
	// zero_value_test row 1 is seeded with: str_col='non-empty', bool_col=true, int_col=42, float_col=3.14
