...
func Example13_QueryAll(ctx context.Context, db *typedb.DB) {
	...
	rows, err := db.QueryAll(ctx, "SELECT TOP 3 id, name, email FROM users ORDER BY id")
	if err != nil {
...
func Example14_QueryRowMap(ctx context.Context, db *typedb.DB, firstUser *User) {
	...
	row, err := db.QueryRowMap(ctx, "SELECT id, name, email FROM users WHERE id = @p1", firstUser.ID)
	if err != nil {
